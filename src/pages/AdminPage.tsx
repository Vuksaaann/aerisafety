...
import { MapPin, Calendar, CheckCircle, XCircle, Clock, Shield, Mail, User } from "lucide-react";
...
const AdminPage = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      <p className="text-muted-foreground mb-8">Moderacija prijava i poruka</p>

      <Tabs defaultValue="reports">
        <TabsList className="mb-6">
          <TabsTrigger value="reports">Prijave građana</TabsTrigger>
          <TabsTrigger value="contacts">Kontakt poruke</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="contacts">
          {contactLoading ? (
            ...
            <p className="text-muted-foreground">Nema kontakt poruka.</p>
          ) : (
            <div className="space-y-4">
              {contactMessages.map((msg: any) => (
                <div key={msg.id} className="bg-card border border-border rounded-lg p-5">
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div className="flex items-center gap-3">
                      <User size={16} className="text-muted-foreground" />
                      <span className="font-bold text-foreground">{msg.name}</span>
                    </div>
                    <Badge variant="secondary">{msg.inquiry_type}</Badge>
                  </div>
                  <div className="flex items-center gap-4 text-xs text-muted-foreground mb-3">
                    <span className="flex items-center gap-1"><Mail size={12} />{msg.email}</span>
                    <span className="flex items-center gap-1"><Calendar size={12} />{format(new Date(msg.created_at), "dd.MM.yyyy HH:mm")}</span>
                  </div>
